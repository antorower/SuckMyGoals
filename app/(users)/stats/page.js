    ...
    <div className="flex flex-col gap-4 p-8 w-full">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="grid grid-cols-12 max-w-[400px] animate-pulse">
          ...
          <div className="col-span-3">{numberOfTraders > 0 ? `${Math.round((((byCompany["The5ers"]?.length || 0) + (byCompany["Funding Pips"]?.length || 0) + (byCompany["Funded Next"]?.length || 0) + (byCompany["Funded Next Stellar"]?.length || 0) + (byCompany["Maven"]?.length || 0)) / (numberOfTraders * 3)) * 100)}%` : "0%"}</div>
        </div>
      </div>
