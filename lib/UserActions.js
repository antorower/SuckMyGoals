...
export const AddUserAccount = async (userId, accountId) => {
  try {
    await dbConnect();
    let user = await User.findById(userId);
    if (!user) throw new Error("User not found");
    await user.addAccount(accountId);
    return { error: false, message: "Account added successfully" };
  } catch (error) {
    console.log("Error adding account to user: ", error);
    return { error: true, message: "Internal error occurred, please try again" };
  }
};

export const RemoveUserAccount = async (userId, accountId) => {
  try {
    await dbConnect();
    let user = await User.findById(userId);
    if (!user) throw new Error("User not found");
