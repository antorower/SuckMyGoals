...
export const GetNewUsers = async () => {
  try {
    ...
    console.error("Error getting new users. File: UserActions - Function: GetNewUsers", error);
    return { error: true, message: error.message };
  }
};
...
export const GetUserProfitsByClerkId = async (id) => {
  try {
    await dbConnect();
    const profits = await User.findOne({ clerkId: id }).select("profits");
    return profits.profits;
  } catch (error) {
    console.error("Error getting user profits. File: UserActions - Function: GetUserProfitsByClerkId", error);
    return { error: true, message: error.message };
  }
};
...
export const AddBeneficiary = async (userId, beneficiaryId, percentage) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    ...
    await user.addBeneficiary(beneficiaryId, percentage);

    return { message: "Beneficiary added successfully" };
  } catch (error) {
    console.error("Error updating user beneficiaries. File: UserActions - Function: AddBeneficiary", error);
    return { error: true, message: error.message };
  }
};
...
export const RemoveBeneficiary = async (userId, beneficiaryId) => {
  try {
    ...
    await user.removeBeneficiary(beneficiaryId);

    return { message: "Beneficiary removed successfully" };
  } catch (error) {
    console.error("Error updating user beneficiaries. File: UserActions - Function: RemoveBeneficiary", error);
    return { error: true, message: error.message };
  }
};
...
export const AcceptUser = async (userId) => {
  try {
    ...
    await user.acceptUser(user.clerkId);

    return { message: "User accepted successfully" };
  } catch (error) {
    console.error("Error accepting user. File: UserActions - Function: AcceptUser", error);
    return { error: true, message: error.message };
  }
};
...
export const IsLeader = async (userId, leaderId) => {
  try {
    ...
    const isLeader = await user.isLeader(leaderId);

    return { isLeader: isLeader };
  } catch (error) {
    console.error("Error getting if user is leader. File: UserActions - Function: IsLeader", error);
    return { error: true, message: error.message };
  }
};

export const IsBeneficiary = async (userId, leaderId) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    ...
    const isBeneficiary = await user.isBeneficiary(leaderId);

    return { isBeneficiary: isBeneficiary };
  } catch (error) {
    console.error("Error getting if user is leader. File: UserActions - Function: IsLeader", error);
    return { error: true, message: error.message };
  }
