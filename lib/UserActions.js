...
export const GetUserById = async (userId) => {
  try {
    ...
    return await User.findById(userId)
      ...
      .populate({ path: "accounts", populate: { path: "openTrade" } });
  } catch (error) {
...
export const AddUserCompany = async (userId, companyId) => {
  try {
    ...
    if (!user) throw new Error("User not found");
    if (!user.activeCompanies.includes(companyId)) {
      user.activeCompanies.push(companyId);
      await user.save();
      return { message: "Company added successfully" };
    }
    return { message: "Company is already included" };
  } catch (error) {
    console.error("Error add active company to user. File: UserActions - Function: RemoveUserCompany", error);
    return { error: true, message: error.message };
  }
};
...
export const RemoveUserCompany = async (userId, companyId) => {
  try {
    ...
    if (!user) throw new Error("User not found");
    if (user.activeCompanies.includes(companyId)) {
      user.activeCompanies.pull(companyId);
      await user.save();
      return { message: "Company removed successfully" };
    }
    return { message: "Company is already removed" };
  } catch (error) {
    console.error("Error remove active company from user. File: UserActions - Function: RemoveUserCompany", error);
    return { error: true, message: error.message };
  }
};
