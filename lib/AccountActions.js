...
export const GetAllAccountsLight = async () => {
  try {
    await dbConnect();
    const accounts = await Account.find({ status: { $in: ["Live", "NeedUpgrade", "WaitingPayout", "PayoutRequestDone", "MoneySended", "Review"] } }).select("company number _id phaseWeight balance status payoutRequestDate capital");
    return accounts;
  } catch (error) {
