...
export const AccountInitialization = async (userId, companyName, capital, investment, investor) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    const newAccount = new Account();
    await newAccount.accountInitialization(userId, companyName, capital, investment, investor);
...
export const RegisterAccountNumber = async (accountId, accountNumber) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    const account = await Account.findById(accountId).populate("investor");
    const cost = account.investment ? GetAccountCost(account.company, account.capital) : 0;
    if (cost === 1000000) throw new Error("Wrong company or capital");
    if (!account) throw new Error("Account not found");
    await account.accountPurchased(accountNumber);
    if (account.investor) await account.investor.addProfits(cost, account.user, account._id, "Investment");
...
const GetAccountCost = (company, capital) => {
  if (company === "Funded Next Stellar") {
    if (capital === 6000) {
      return 63;
    }
  }
...
export const GetAllAccountsLight = async () => {
  try {
    await dbConnect();
    const accounts = await Account.find({ status: { $in: ["Live", "NeedUpgrade", "WaitingPayout", "PayoutRequestDone", "MoneySended", "Review"] } }).select("company number _id phaseWeight balance status payoutRequestDate capital eventsTimestamp tradesDisabled metadata");
    return accounts;
  } catch (error) {
...
export const GetAllAccountsStats = async () => {
  try {
    await dbConnect();
    const accounts = await Account.find({
      status: { $in: ["WaitingPurchase", "Live", "NeedUpgrade", "WaitingPayout", "PayoutRequestDone", "MoneySended"] },
    }).select("company capital balance phaseWeight status"); // Επιλέγουμε μόνο τα επιθυμητά πεδία
    return accounts;
  } catch (error) {
    console.error("Error getting all accounts. File: AccountActions - Function: GetAllAccountsLight ", error);
    return { error: true, message: error.message };
...
export const UpdatePayoutRequestDate = async (accountId, day, month, year) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    ...
    await account.updatePayoutRequestDate(day, month, year);
  ...
  try {
    ...
    */
    ...
    if (matchingTrade) {
      ...
      if (account.company === "Funded Next" && account.balance === account.capital && account.phaseWeight === 3) {
        newTrade.trade.takeProfit = Math.round(newTrade.trade.takeProfit * 0.7);
      }
...
export const MoneySended = async (mongoId, accountId, accountBalance, payoutAmount, profitShare, refund, finalAmount) => {
  try {
    await dbConnect();
    revalidatePath("/", "layout");
    const account = await Account.findById(accountId);
    ...
    await newPayout.createNewPayout(mongoId, accountId, accountBalance, payoutAmount, profitShare, refund, finalAmount);

    return { message: "Profits Profits Profits" };
  } catch (error) {
