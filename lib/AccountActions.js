...
export const GetAllAccountsLight = async () => {
  try {
    await dbConnect();
    const accounts = await Account.find({ status: { $in: ["Live", "NeedUpgrade", "WaitingPayout", "PayoutRequestDone", "MoneySended"] } }).select("company number _id phaseWeight balance status lastTrade payoutRequestDate");
    return accounts;
  } catch (error) {
